-- Migration: 20260717110000_tmos_matching_itinerary_foundation
...
DROP TRIGGER IF EXISTS update_tmos_business_partners_updated_at ON public.tmos_business_partners;
CREATE TRIGGER update_tmos_business_partners_updated_at
  BEFORE UPDATE ON public.tmos_business_partners
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

DROP TRIGGER IF EXISTS update_tmos_delegate_matches_updated_at ON public.tmos_delegate_matches;
CREATE TRIGGER update_tmos_delegate_matches_updated_at
  BEFORE UPDATE ON public.tmos_delegate_matches
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

DROP TRIGGER IF EXISTS update_tmos_itinerary_items_updated_at ON public.tmos_itinerary_items;
CREATE TRIGGER update_tmos_itinerary_items_updated_at
  BEFORE UPDATE ON public.tmos_itinerary_items
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

CREATE INDEX IF NOT EXISTS idx_tmos_business_partners_event ON public.tmos_business_partners(event_id, status);
CREATE INDEX IF NOT EXISTS idx_tmos_delegate_matches_registration ON public.tmos_delegate_matches(event_registration_id, status);
CREATE INDEX IF NOT EXISTS idx_tmos_delegate_matches_event ON public.tmos_delegate_matches(event_id, scheduled_at);
CREATE INDEX IF NOT EXISTS idx_tmos_itinerary_registration ON public.tmos_itinerary_items(event_registration_id, start_at);
CREATE INDEX IF NOT EXISTS idx_tmos_itinerary_event ON public.tmos_itinerary_items(event_id, start_at);
...
-- Migration: 20260717120000_tmos_travel_visa_operations
...
DROP TRIGGER IF EXISTS update_tmos_travel_cases_updated_at ON public.tmos_travel_cases;
CREATE TRIGGER update_tmos_travel_cases_updated_at
  BEFORE UPDATE ON public.tmos_travel_cases
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

CREATE INDEX IF NOT EXISTS idx_tmos_travel_cases_event ON public.tmos_travel_cases(event_id, visa_status, flight_status);
CREATE INDEX IF NOT EXISTS idx_tmos_travel_cases_registration ON public.tmos_travel_cases(event_registration_id);
